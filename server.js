...
let onBuildError = (err) => { 
    logger.error('Error while building the project', err);
    process.exit(1);
    return;
}

compiler.run(function(err, stats) {
    logger.info('Compiled successfully', stats.toString({ colors: true}));
...
const startServer = (buildStats) => {
  ...
  const server = app.listen(appConfig.port, appConfig.ip, () => {
...
compiler.watch({ // watch options:
    ...
    if(err){ return onBuildError(err); }
    logger.info('RE-Compiled successfully', stats.toString({ colors: true}));
    // return startServer(stats);
});
